...
class RequestVerificationFragment : Fragment(), BottomSheetListFragment.SelectLanguage,View.OnClickListener {
    ...
    private fun setupViewModel() {
         loginModel = ViewModelProvider(this@RequestVerificationFragment).get(SignupModel::class.java)
    ...
    private fun checkValidation() {
        ...
        when {
            fev_edit_first_name?.text.toString().isNullOrEmpty() -> {
                MyUtils.showSnackbar(mActivity!!, resources.getString(R.string.msg_empty_first_name), frv_rl_main)
            }

            fev_edit_last_name?.text.toString().isNullOrEmpty() -> {
                MyUtils.showSnackbar(mActivity!!, resources.getString(R.string.msg_empty_last_name), frv_rl_main)
            }

            fev_edit_known_as?.text.toString().isNullOrEmpty() -> {
                MyUtils.showSnackbar(mActivity!!, resources.getString(R.string.msg_empty_known_as), frv_rl_main)
            }
    ...
    private fun getSendVerification() {
            ...
            loginModel.userRegistration(mActivity!!, false, jsonArray.toString(), "sendVerification")
                                ...
                                if (loginPojo != null) {
                                    if (loginPojo.get(0).status.equals("true", true)) {
                                        ...
                                        try {
                                            ...
                                            Handler().postDelayed({
                                                frv_nl_main?.visibility = View.GONE
                                                frv_ll_requested_submitted?.visibility = View.VISIBLE                                            }, 1000)
                                        } catch (e: Exception) {
                                            e.printStackTrace()
                                        }
                                    } else {
...
//                            MyUtils.dismissProgressDialog()
                            ...
                            try {
                                if (MyUtils.isInternetAvailable(mActivity!!)) {
                                    MyUtils.showSnackbar(mActivity!!, resources.getString(R.string.error_crash_error_message), frv_rl_main)
                                } else {
                                    MyUtils.showSnackbar(mActivity!!, resources.getString(R.string.error_common_network), frv_rl_main)
                                }
                            } catch (e: Exception) {
                                e.printStackTrace()
                            }
