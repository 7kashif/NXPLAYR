...
class ProfileSummaryFragment : Fragment(), View.OnClickListener {
    ...
    override fun onResume() {
        ...
        if (sessionManager != null && sessionManager?.LanguageLabel != null) {
            if (!sessionManager?.LanguageLabel?.lngProfileSummary.isNullOrEmpty())
                tvToolbarTitle.text = sessionManager?.LanguageLabel?.lngProfileSummary
            if (!sessionManager?.LanguageLabel?.lngProfileTitle.isNullOrEmpty())
                prof_title.text = sessionManager?.LanguageLabel?.lngProfileTitle
            if (!sessionManager?.LanguageLabel?.lngPosition.isNullOrEmpty())
                prof_position.text = sessionManager?.LanguageLabel?.lngPosition
            if (!sessionManager?.LanguageLabel?.lngOrganization.isNullOrEmpty())
                prof_org.text = sessionManager?.LanguageLabel?.lngOrganization
            if (!sessionManager?.LanguageLabel?.lngWhoAmI.isNullOrEmpty())
                prof_who_am.text = sessionManager?.LanguageLabel?.lngWhoAmI
    ...
    private fun profileSummary() {
        ...
        FirebaseMessaging.getInstance().token.addOnCompleteListener(OnCompleteListener { task ->
            ...
            var signup =
                ViewModelProvider(this@ProfileSummaryFragment).get(SignupModelV2::class.java)
            signup.userUpdateProfile(jsonArray.toString())
            signup.userUpdateProfile
                .observe(
                    viewLifecycleOwner
                ) { loginPojo ->
                    if (loginPojo != null) {
