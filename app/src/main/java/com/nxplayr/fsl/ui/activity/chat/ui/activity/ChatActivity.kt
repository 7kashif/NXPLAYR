...
import com.nxplayr.fsl.ui.activity.chat.ui.adapter.*
import com.nxplayr.fsl.ui.activity.chat.ui.adapter.listeners.AttachClickListener
import com.nxplayr.fsl.ui.activity.chat.ui.adapter.listeners.MessageLongClickListener
import com.nxplayr.fsl.ui.activity.chat.ui.dialog.ProgressDialogFragment
import com.nxplayr.fsl.ui.activity.chat.ui.views.AttachmentPreviewAdapterView
import com.nxplayr.fsl.ui.activity.chat.utils.SharedPrefsHelper
import com.nxplayr.fsl.ui.activity.chat.utils.SystemPermissionHelper
import com.nxplayr.fsl.ui.activity.chat.utils.chat.CHAT_HISTORY_ITEMS_PER_PAGE
import com.nxplayr.fsl.ui.activity.chat.utils.chat.ChatHelper
import com.nxplayr.fsl.ui.activity.chat.utils.imagepick.OnImagePickedListener
import com.nxplayr.fsl.ui.activity.chat.utils.imagepick.pickAnImage
import com.nxplayr.fsl.ui.activity.chat.utils.qb.*
import com.nxplayr.fsl.ui.activity.chat.utils.shortToast
import com.nxplayr.fsl.utils.showSnackbar
...
class ChatActivity : BaseActivity(), OnImagePickedListener, QBMessageStatusListener, DialogsManager.ManagingDialogsCallbacks {
    ...
    private fun initMessagesRecyclerView() {
        ...
        if (qbChatDialog.name != null) {
            if (qbChatDialog.name.contains("profilename: ") ||
                    qbChatDialog.name.contains("profilepicture: ") ||
                    qbChatDialog.name.contains("countryname: ") ||
                    qbChatDialog.name.contains("jobtitle: ")) {
                var splitString: Array<String>? = null
                if (!TextUtils.isEmpty(qbChatDialog.name)) {
                    splitString = qbChatDialog.name.split(", ").toTypedArray()
                }
                if (splitString != null && splitString.size > 0) {
                    for (i in splitString.indices) {
                        if (splitString[i].contains("profilename: ")) {
                            userName = splitString[i].replace("profilename: ", "")
                        }
                        if (splitString[i].contains("profilepicture: ")) {
                            userProfilePicture = splitString[i].replace("profilepicture: ", "")
                        }
                        if (splitString[i].contains("countryname: ")) {
                            userLocation = splitString[i].replace("countryname: ", "")
                        }
                    }
                }
            } else {
                userName = qbChatDialog.name
            }
        }

       if (!userLocation.isEmpty()) {
            customTextView(chatDetailTextOpponentName, userName, " \u2022 $userLocation")

        } else if (userLocation.isEmpty()) {
            customTextView(chatDetailTextOpponentName, userName, "")
        }

        if (!userProfilePicture.isEmpty())
            chatDetailImageOpponent.setImageURI(Uri.parse(userProfilePicture))
    ...
    private fun customTextView(view: AppCompatTextView, text1: String, text2: String) {
        val spanTxt = SpannableStringBuilder("" + text1)
        spanTxt.append(text2)
        spanTxt.setSpan(object : ClickableSpan() {
            override fun onClick(widget: View) {}
            override fun updateDrawState(ds: TextPaint) {
                ds.isUnderlineText = false
            }
        }, spanTxt.length - text2.length, spanTxt.length, 0)
        spanTxt.setSpan(ForegroundColorSpan(
                applicationContext.resources.getColor(R.color.textcolor02)),
                spanTxt.length - text2.length,
                spanTxt.length,
                0)
