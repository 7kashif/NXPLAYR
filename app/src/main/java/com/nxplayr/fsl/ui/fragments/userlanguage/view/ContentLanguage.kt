...
class ContentLanguage : Fragment(), View.OnClickListener {
    ...
    override fun onResume() {
        ...
        if (sessionManager != null && sessionManager?.LanguageLabel != null) {
            ...
            if (!sessionManager?.LanguageLabel?.lngTranslation.isNullOrEmpty())
                select_lang.text = sessionManager?.LanguageLabel?.lngTranslation
            if (!sessionManager?.LanguageLabel?.lngTranslationDetail.isNullOrEmpty())
                select_lang_desc.text = sessionManager?.LanguageLabel?.lngTranslationDetail
            if (!sessionManager?.LanguageLabel?.lngChange.isNullOrEmpty())
                tv_content_language_change.text = sessionManager?.LanguageLabel?.lngChange
        }
    }
        ...
        when (v?.id) {
            R.id.tv_content_language_change -> {
                (activity as MainActivity).navigateTo(
                    ...
                    SelectContentLanguage::class.java.name,
                    true
                )
            }
        }
