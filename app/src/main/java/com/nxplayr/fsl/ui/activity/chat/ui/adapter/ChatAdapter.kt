...
import com.nxplayr.fsl.ui.activity.chat.managers.PROPERTY_NOTIFICATION_TYPE
import com.nxplayr.fsl.ui.activity.chat.ui.activity.PROPERTY_FORWARD_USER_NAME
...
import com.nxplayr.fsl.ui.activity.chat.utils.getDateAsHeaderId
import com.nxplayr.fsl.ui.activity.chat.utils.isAttachmentValid
import com.nxplayr.fsl.ui.activity.chat.utils.qb.PaginationHistoryListener
import com.nxplayr.fsl.ui.activity.chat.utils.qb.QbUsersHolder
...
class ChatAdapter(protected var context: Context,
    ...
    private fun fillTextMessageHolder(holder: NewMessageViewHolder, chatMessage: QBChatMessage, isIncomingMessage: Boolean) {
        ...
        if (forwardedFromName != null) {
            ...
            holder.tvTextForwardedFromUser?.text = getSenderName(forwardedFromName)
        } else {
            ...
            when {
                photo || image -> {
                    ...
                    if (forwardedFromName != null) {
                        ...
                        holder.tvImageForwardedFromUser?.text = getSenderName(forwardedFromName)
                    } else {
                ...
                video -> {
                    ...
                    if (forwardedFromName != null) {
                        ...
                        holder.llVideoForwardedFromUser?.text =  getSenderName(forwardedFromName)
                    } else {
                ...
                file -> {
                    ...
                    if (forwardedFromName != null) {
                        ...
                        holder.llFileForwardedFromUser?.text = getSenderName(forwardedFromName)
                    } else {
    ...
    private fun getSenderName(chatMessage: QBChatMessage): String {
        ...
        var sender:QBUser?=null
        if(chatMessage.senderId!=null)
        {
         sender=QbUsersHolder.getUserById(chatMessage.senderId!!)!!
        }

        var fullName = ""
        if (sender!=null &&!TextUtils.isEmpty(sender?.fullName)) {
            fullName = sender!!.fullName
            var userProfilePicture = ""

            if (sender.fullName != null) {
                if (sender.fullName .contains("profilename: ") ||
                        sender.fullName.contains("profilepicture: ") ||
                        sender.fullName.contains("countryname: ") ||
                        sender.fullName.contains("jobtitle: ")) {
                    var splitString: Array<String>? = null
                    if (!TextUtils.isEmpty(sender.fullName)) {
                        splitString = sender.fullName.split(", ").toTypedArray()
                    }
                    if (splitString != null && splitString.size > 0) {
                        for (i in splitString.indices) {
                            if (splitString[i].contains("profilename: ")) {
                                fullName = splitString[i].replace("profilename: ", "")
                            }
    ...
    private fun getSenderName(fullnameOriginal: String): String {
        var fullName = ""
        if (!TextUtils.isEmpty(fullnameOriginal)) {
            fullName = fullnameOriginal
            var userProfilePicture = ""

            if (fullnameOriginal != null) {
                if (fullnameOriginal .contains("profilename: ") ||
                        fullnameOriginal.contains("profilepicture: ") ||
                        fullnameOriginal.contains("countryname: ") ||
                        fullnameOriginal.contains("jobtitle: ")) {
                    var splitString: Array<String>? = null
                    if (!TextUtils.isEmpty(fullnameOriginal)) {
                        splitString =fullnameOriginal.split(", ").toTypedArray()
                    }
                    if (splitString != null && splitString.size > 0) {
                        for (i in splitString.indices) {
                            if (splitString[i].contains("profilename: ")) {
