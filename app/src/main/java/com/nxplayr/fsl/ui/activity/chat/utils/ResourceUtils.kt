...
private fun getCircleColor(@IntRange(from = RANDOM_COLOR_START_RANGE.toLong(), to = RANDOM_COLOR_END_RANGE.toLong())
    ...
    val colorId = MyApplication.instance.resources.getIdentifier(colorIdName, "color", MyApplication.instance.packageName)
    return MyApplication.instance.resources.getColor(colorId)
}

fun getGreyCircleDrawable(): Drawable {
    return getColoredCircleDrawable(MyApplication.instance.resources.getColor(R.color.color_grey))
}

private fun getColoredCircleDrawable(@ColorInt color: Int): Drawable {
    val drawable = MyApplication.instance.resources.getDrawable(R.drawable.shape_circle) as GradientDrawable
