 ...
 */
class TrendingItemDocumentAdapter(var context: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder?>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (holder is ImgHolder) {
         ...
         //   holder1. useruploadDocumentWebview.getSettings().setLoadsImagesAutomatically(true);
            if(!trendingFeedDatum?.postSerializedData.isNullOrEmpty() && trendingFeedDatum?.postSerializedData?.get(0)?.albummedia?.get(0)?.albummediaFile?.contains(".pdf")!!){
                 holder1.useruploadImageview.setImageResource(R.drawable.pdf_icon)
             }
             else if(!trendingFeedDatum?.postSerializedData.isNullOrEmpty() && trendingFeedDatum?.postSerializedData?.get(0)?.albummedia?.get(0)?.albummediaFile?.contains(".docx")!!)
             {
                 holder1.useruploadImageview.setImageResource(R.drawable.word_file_icon)
             }
             else if(!trendingFeedDatum?.postSerializedData.isNullOrEmpty() && trendingFeedDatum?.postSerializedData?.get(0)?.albummedia?.get(0)?.albummediaFile?.contains(".xlsx")!!)
             {
                 holder1.useruploadImageview.setImageResource(R.drawable.word_file_icon)

             }else if(!trendingFeedDatum?.postSerializedData.isNullOrEmpty() && trendingFeedDatum?.postSerializedData?.get(0)?.albummedia?.get(0)?.albummediaFile?.contains(".pptx")!!)
             {
