...
import com.nxplayr.fsl.utils.getFilePath
import java.io.File
...
class GetFilepathFromUriTask(fragmentManager: FragmentManager,
    ...
    override fun performInBackground(vararg params: Intent): File {
        ...
        if (SCHEME_CONTENT.equals(uriScheme, ignoreCase = true) && !isFromGoogleApp && !isKitKatAndUpper && uri != null) {
            ...
            val cursor = MyApplication.instance.contentResolver.query(uri, filePathColumn, null, null, null)
            cursor?.let {
                if (it.count > 0) {
                    it.moveToFirst()
            ...
            uri?.let {
                imageFilePath = getFilePath(MyApplication.instance, it)
            }
        }
