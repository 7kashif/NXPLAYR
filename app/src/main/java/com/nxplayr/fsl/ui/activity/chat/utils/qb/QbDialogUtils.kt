    ...
    if (dialog.type == QBDialogType.GROUP) {
        ...
        user?.let {
            ...
            if (dialog.name != null) {
                ...
                if (dialog.name.contains("profilename: ") ||
                        dialog.name.contains("profilepicture: ") ||
                        dialog.name.contains("countryname: ") ||
                        dialog.name.contains("jobtitle: ")) {
                    var splitString: Array<String>? = null
                    if (!TextUtils.isEmpty(dialog.name)) {
                        splitString = dialog.name.split(", ").toTypedArray()
                    }
                    if (splitString != null && splitString.size > 0) {
                        for (i in splitString.indices) {
                            if (splitString[i].contains("profilename: ")) {
                                result = splitString[i].replace("profilename: ", "")
                            }
