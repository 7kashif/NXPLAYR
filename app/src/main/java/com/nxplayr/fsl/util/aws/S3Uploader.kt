package com.nxplayr.fsl.util.aws
...
class S3Uploader(var context: Context) {
    ...
    fun initUpload(filePath: String?, fileName: String?, contentType : String) {
        val file = File(filePath)
        val myObjectMetadata = ObjectMetadata()
        myObjectMetadata.contentType = contentType
        val observer = transferUtility!!.upload(
            AWSKeys.BUCKET_NAME, fileName,
            file
        )
